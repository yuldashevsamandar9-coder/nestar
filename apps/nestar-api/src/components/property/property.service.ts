...
export class PropertyService {
	...
	public async getProperty(memberId: ObjectId, propertyId: ObjectId): Promise<Property> {
		...
		const targetProperty = await this.propertyModel.findOne(search).lean().exec(); // lean targetproperni modifier qilis
		if (!targetProperty) throw new InternalServerErrorException(Message.NO_DATA_FOUND);

		if (memberId) {
			const viewInput: ViewInput = {
				memberId: memberId as unknown as Types.ObjectId,
				viewRefId: propertyId as unknown as Types.ObjectId,
			...
			// meLiked
			// meLiked
			const likeInput = { memberId: memberId as any, likeRefId: propertyId as any, likeGroup: LikeGroup.PROPERTY };
